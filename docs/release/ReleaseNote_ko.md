...
### 🔄 사이드바 업데이트 확인 오류 단일행 처리 및 레이아웃 안정화
- **세로 레이아웃 흔들림 및 크기 왜곡 방지**:
  - 사이드바 하단 업데이트 확인 피드백 로직을 전면 개선하여, 오프라인이나 시간 초과 시 긴 오류 메시지가 줄바꿈되어 사이드바 영역을 밀어 올리던 UI 왜곡 현상을 해결했습니다.
  - Cocoa 네이티브 `NSLineBreakByTruncatingTail` 말줄임표 처리와 고정 높이 라벨을 적용하여 사이드바의 기하학적 치수를 엄격히 유지합니다.
  - 지능형 오류 분류기(`format_update_error`)를 도입하여 복잡한 네트워크 및 HTTP 예외를 직관적인 단일행 메시지(시간 초과, 네트워크 오류, HTTP 상태 코드 등)로 변환합니다.
  - 5초 후 오류 메시지가 자동으로 사라지는 타이머를 추가하여 사이드바를 항상 깔끔한 기본 상태로 복원합니다.

### 🌐 다국어 업데이트 오류 메시지 완비
- **9개 언어 지원**:
  - 업데이트 오류 처리를 위한 4가지 현지화 키(`update_error_short`, `update_error_timeout`, `update_error_network`, `update_error_http`)를 전 언어에 적용했습니다.

### 🧪 자동화 테스트 스위트 확장 및 신뢰성 확보
- **테스트 케이스 추가**:
  - 오류 메시지 포맷팅, 단일행 말줄임표 및 비동기 자동 초기화 전용 테스트를 구축했습니다(`test_sidebar_update.py`, `test_i18n_update_keys.py`).
  - 전체 자동화 테스트 스위트를 **659개**(통과율 100%)로 확장했습니다.
...
- **맞춤형 복제본 정보 기록**:
  - `CloneRecord` 및 복제 편집 창(`CloneEditWindow`)에 `notes`(메모) 필드를 추가하여 계정 이름, 용도, 지역 등의 상세 정보를 자유롭게 기록할 수 있습니다.
  - 복제 목록(`CloneListView`) 테이블에 전용 '메모' 컬럼을 추가하고 헤더 클릭 오름차순/내림차순 정렬을 지원합니다.
  - 상단 툴바 검색창에서 메모 내용에 대한 퍼지 검색 매칭을 지원하여 대량의 복제본 중 원하는 인스턴스를 즉각 찾을 수 있습니다.
  - 카드 뷰(`CloneCard`)에 메모가 있는 경우 요약 내용을 직관적으로 표시합니다.
  - 메모 레이블 및 입력 안내 문구에 대해 9개 언어 현지화를 지원합니다.

### 📐 메인 윈도우 및 사이드바 레이아웃 사용성 개선
- **시각적 여유 공간 및 가독성 향상**:
  - 메인 윈도우 기본 크기를 `1300 x 760`(기존 1200 x 680)으로 확장하여 카드 그리드 및 다단 테이블을 훨씬 여유롭게 탐색할 수 있습니다.
  - 좌측 네비게이션 사이드바 너비를 200px에서 240px로 확장하여 메뉴 항목, 복제본 수 배지 및 업데이트 상태가 자연스럽게 표시됩니다.

### 🔬 동적 주입 라이브러리 Clang 실제 컴파일 검증
- **저수준 훅 안정성 극대화**:
  - 주입되는 동적 라이브러리에 대해 실제 `clang` 컴파일러를 호출하는 엔드투엔드 테스트를 추가했습니다:
    - Cocoa / POSIX interpose 후킹 라이브러리(`libatbclone_hook`).
    - Lark(Feishu) 및 ChatGPT 전용 다중 실행 격리 동적 라이브러리.
    - 환경 변수 주입 라이브러리(`libatbclone_env`).
  - 구문, 매크로 또는 헤더 변경으로 발생할 수 있는 빌드 및 로드 오류를 원천 방지합니다.

### 🧪 테스트 스위트 확장 및 디스플레이 독립 검증
- **테스트 견고성 강화**:
  - GUI 통합 테스트에서 디스플레이 해상도에 종속되던 창 크기 단언을 정리하여 헤드리스 CI 및 다양한 다중 모니터 환경에서 결정론적인 실행을 보장합니다.
  - 자동화 테스트 케이스를 총 **648개**(통과율 100%)로 확장했습니다.

---

## [v2.1.1] - 2026-09-18

### 🔧 C 언어 런처 변수 스코프 수정 및 컴파일 안정성 강화
- **Lark / ChatGPT 등 복제본 런처 컴파일 오류 해결**:
  - 커스텀 동적 라이브러리 훅(Lark/Feishu, ChatGPT 등)을 적용할 때, 생성된 C 런처 내 `hook_block` 코드와 런처 자체의 `dir` 변수가 동일한 스코프에 위치하여 발생하던 중복 정의 컴파일 오류를 수정했습니다.
  - C 런처 생성기 내에서 `hook_block` 로직을 독립된 블록 `{ ... }`으로 격리하여 변수명 충돌을 원천 차단했습니다.
  - 실제 `clang` 컴파일러를 호출하여 Lark 및 ChatGPT 런처를 검증하는 엔드투엔드 테스트를 추가했습니다.

### 🛡️ 프로세스 이름 변경 안정성 강화 및 비표준 번들 예외 처리
- **안전한 오류 격리 및 자동 폴백**:
  - 실행 바이너리 이름 변경 및 `Info.plist` 동기화 로직을 보강하여 비표준 번들 구조, plist 누락, 손상된 바이너리 환경에서도 안전하게 작동하도록 예외 처리를 강화했습니다.
  - 예외 발생 시 명확한 경고 로그를 기록하며 기본 실행 경로로 매끄럽게 폴백되어 복제 생성이 중단되지 않습니다.
  - 비정상 번들 및 폴백 시나리오를 다루는 전용 테스트 케이스를 보강했습니다.

### 🎨 복제 생성 마법사 아이콘 미리보기 널 안전성 확보
- **UI 비정상 종료 방지**:
  - 생성 마법사(`WizardWindow`)의 아이콘 비동기 미리보기 로직에 방어적 널 검사를 추가하여 메타데이터가 불완전한 앱 스캔 시 발생할 수 있는 UI 예외를 방지했습니다.

### 🧪 품질 보증 및 자동화 테스트 확장
- **테스트 케이스 대폭 확충**:
  - 전체 자동화 테스트 스위트를 **645개**(통과율 100%)로 확장했습니다.

---

## [v2.1.0] - 2026-09-17

### 🚀 하드 복제본 고유 프로세스 명명 및 프록시 규칙 라우팅 지원
- **프로세스 이름 기반 네트워크 프록시 라우팅**:
  - 하드 복제(Hard Clone) 엔진이 복제본마다 고유한 실행 바이너리 이름을 자동 생성하고 `CFBundleExecutable`을 동기화(예: `<앱이름>_<복제본이름>`)하여 독립 프로세스로 식별할 수 있습니다.
  - Clash, Surge, Loon, Stash, Quantumult X 등의 네트워크 프록시 클라이언트에서 프로세스 이름 기반 라우팅 규칙(`PROCESS-NAME,<복제프로세스명>,<노드/정책>`)을 완벽히 지원하여 복제본별 독립 프록시 설정이 가능합니다.
  - Electron 및 Chromium 기반 도우미 앱(Helper App)의 경로를 안전하게 보존하여 프로세스 간 IPC 통신의 무결성을 보장합니다.
  - 복제본 편집, 버전 업데이트 또는 재생성 시에도 고유 프로세스 명칭과 커스텀 아이콘이 안전하게 유지됩니다.

### 🎨 복제 생성 마법사 내 커스텀 앱 아이콘 지정
- **시각적 맞춤형 경험 제공**:
  - 생성 마법사(`WizardWindow`)에 커스텀 아이콘 선택 기능을 추가하여 `.icns` 및 `.png` 이미지 파일을 간편하게 등록할 수 있습니다.
  - 마법사 내 실시간 아이콘 미리보기 및 원클릭 기본 아이콘 복원을 지원합니다.
  - 리소스 디렉터리(`Contents/Resources/`) 내 아이콘 교체 및 설정을 자동화하여 향후 업데이트 시에도 영구 보존됩니다.
  - 지원하는 9개 언어 전체에 걸쳐 관련 UI 레이블 및 안내 문구를 완벽히 적용했습니다.

### 🔄 사이드바 하단 상시 업데이트 확인 및 진행률 표시
- **직관적인 업데이트 환경 통합**:
  - 업데이트 확인 인터페이스를 환경설정 화면에서 탐색 사이드바 하단으로 이전하여 접근성을 대폭 높였습니다.
  - 새로고침 아이콘, 실시간 버전 피드백 및 인라인 다운로드 진행률(%) 표시 바를 제공합니다.
  - 설정 화면을 간소화하여 핵심 기본 설정에 집중할 수 있도록 정리했습니다.

### 🧪 품질 보증 및 자동화 테스트 확장
- **테스트 케이스 대폭 확충**:
  - 고유 프로세스 이름, Helper 경로 보존, 아이콘 업데이트 영속성 및 사이드바 UI 상태 테스트를 추가했습니다(`test_process_names.py`, `test_update_icon.py`).
  - 전체 자동화 테스트 스위트를 **642개**(통과율 100%)로 확장했습니다.

---

## [v2.0.0] - 2026-09-17

### 🏥 환경 진단 (Doctor) 및 디렉터리 읽기/쓰기 권한 검증
- **주요 저장소 디렉터리 권한 정밀 검사**:
  - 환경 진단 시스템(CLI `atbclone doctor` 및 GUI `DoctorView`)을 전면 개선하여 애플리케이션 디렉터리(`~/ATBClone/Apps`)와 데이터 저장 디렉터리(`~/ATBClone/Data`)의 실시간 읽기/쓰기 권한 검증 기능을 추가했습니다.
  - 안전한 임시 프로브 파일을 활용해 복제 생성 및 인스턴스 실행 전 권한 문제를 사전에 감지하고 런타임 오류를 방지합니다.
  - 명확한 상태 배지, 상세한 시스템 오류 원인 및 직관적인 권한 복구 안내(`chmod u+rwx <path>`)를 제공합니다.
  - `core.config` 모듈에 `check_directory_access()`, `get_apps_dir()`, `get_data_dir()` 유틸리티 함수를 추가했습니다.
  - 9개 국어 i18n 모듈에 진단 상태 레이블, 세부 설명 및 수정 가이드 문구를 완벽히 구축했습니다.

### 📐 공식 문서 포털 KaTeX 수식 렌더링 및 레이아웃 개선
- **수식 타이포그래피 지원**:
  - 공식 문서 사이트(`clone.aitobox.com`)의 9개 언어 에디션 전반에 KaTeX 수식 렌더링 엔진을 탑재하여 기술 원리 수식 표기를 최적화했습니다.
  - LaTeX 디스플레이 수식의 여백 및 렌더링 레이아웃을 다듬어 가독성을 높였습니다.

### ⚡ 아키텍처 완성도 및 품질 보증 (v2.0.0 Milestone)
- **APFS 고성능 엔진 및 동시성 안전성 확립**:
  - v2.0 아키텍처 토대 공고화: APFS Copy-on-Write(`cp -Rc`) 제로 공간 복제, 하드웨어 SIMD 바이너리 분석, `flock` 기반 원자적 상태 지속성.
  - 디렉터리 권한 진단 관련 단위 테스트 및 GUI 테스트 추가(`test_cmd_doctor.py`, `test_config.py`, `test_probe_and_doctor_ui.py`).
  - 전체 자동화 테스트 스위트를 **614개**(통과율 100%)로 확장했습니다.

---

## [v1.9.0] - 2026-09-16

### ⚡ APFS CoW(Copy-on-Write) 고속 복제 및 엔진 성능 최적화
- **즉각적인 제로 카피 복제**:
  - macOS APFS 파일 시스템에서 Copy-on-Write(`cp -Rc`) 복제 방식을 전면 적용하여 대용량 애플리케이션도 추가 디스크 공간 소모 없이 즉각 복제할 수 있습니다.
  - 비 APFS 볼륨 환경을 위한 안정적인 폴백 구현: 기존 재귀 복사(`cp -R`)로 안전하게 전환되며, 폴백 전 잔여 디렉터리를 자동 정리하여 디렉터리 중첩 손상을 방지합니다.
  - 바이너리 문자열 분석에 하드웨어 SIMD 프리필터를 적용하여 앱 실행 인수 및 환경 변수 감지 속도를 대폭 개선했습니다.
  - 래퍼 스크립트 검사 시 Mach-O 바이너리를 안전하게 건너뛰고 코드 서명 권한(entitlements)을 안전하게 정리합니다.

### 🔒 프로세스 간 파일 잠금 및 원자적 상태 저장
- **경쟁 상태 방지 및 데이터 무결성 보장**:
  - `flock` 기반의 프로세스 간 파일 락을 도입하여 여러 복제 인스턴스 또는 CLI 명령어가 동시 실행될 때 발생할 수 있는 상태 충돌을 원천 차단했습니다.
  - 임시 파일 작성, `fsync` 디스크 플러시 및 `os.replace` 교체를 사용하는 원자적(Atomic) 쓰기를 적용하고 파일 권한을 `0o600`으로 제한하여 데이터 손상 및 무단 접근을 방지합니다.
  - Cocoa interpose 동적 라이브러리 후크에서 스레드 로컬(thread-local) `fake_pw`를 적용하여 멀티스레드 환경의 안전성을 보장합니다.

### 🖥️ GUI / HIG 개선 및 메모리 안정성 강화
- **인터페이스 최적화 및 수명 주기 관리**:
  - `LogsView`에 버퍼 상한을 설정하고 메인 스레드로의 안전한 비동기 디스패치를 적용하여 메모리 누수를 해결했습니다.
  - 애플리케이션 종료 시(`_on_app_exit`) 실시간 로그 리스너를 안전하게 해제하도록 수명 주기 정리를 보강했습니다.
  - 보조 창(마법사, 복제본 편집, 레시피 편집, 상세 정보 창)의 윈도우 레벨을 macOS 표준 레벨(`0`)로 복원하여 다른 앱 위에 비정상적으로 최상위 플로팅되는 현상을 수정했습니다.
  - 다크 모드 대비 개선, 사용자 지정 `base_dir` 설정 지속성 지원, 복제본 삭제 시 키체인 프록시 자격 증명 자동 정리를 구현했습니다.
  - 공식 중국어 제품 명칭을 **ATB分身助手**로 통일했습니다.

### 📚 9개 언어 공식 문서 포털 및 품질 보증
- **공식 문서 사이트 (`clone.aitobox.com`)**:
  - Zensical 기반의 정적 문서 사이트를 구축하고 GitHub Pages 자동 배포 파이프라인을 연동했습니다.
  - 지원하는 9개 언어 전반의 통합 문서 매트릭스, 다국어 내비게이션 및 실제 UI 데모 스크린샷을 제공합니다.
  - 22개의 회귀 및 최적화 감사 테스트를 추가하여 전체 테스트 스위트를 **610개**(통과율 100%)로 확장했습니다.
