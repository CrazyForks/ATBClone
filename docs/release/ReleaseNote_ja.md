...
### 🔄 サイドバー更新確認エラー表示の単行化とレイアウト崩れ防止
- **垂直レイアウトの急激な伸長を防止**:
  - サイドバー下部のアップデート確認フィードバック機構を刷新。ネットワーク障害やタイムアウト発生時に、長文のスタックトレースが複数行に折り返されてサイドバーを縦に押し広げてしまうレイアウト崩れを完全に解消。
  - Cocoa ネイティブの `NSLineBreakByTruncatingTail` 末尾省略モードと固定高さラベルを採用し、サイドバー全体の幾何学的寸法を厳格に保持。
  - リクエスト例外をインテリジェントに分類するエラーパーサー（`format_update_error`）を導入し、複雑な通信エラーを簡潔な単行ローカライズ文字列（タイムアウト、接続異常、HTTP ステータスコード等）に自動要約。
  - 5 秒後にエラー表示を自動消去するタイマーを追加し、サイドバーのクリーンな待機状態を自動復元。

### 🌐 アップデートエラー文言の多言語完全対応
- **全 9 言語の網羅**:
  - アップデートエラー用の 4 つの翻訳キー（`update_error_short`, `update_error_timeout`, `update_error_network`, `update_error_http`）を全言語に追加。

### 🧪 テストスイート拡充と品質保証
- **カバレッジの強化**:
  - エラーフォーマット、末尾省略、自動消去に関する専用テストスイートを追加（`test_sidebar_update.py`, `test_i18n_update_keys.py`）。
  - 自動テストスイートを計 **659 件**（合格率 100%）に拡大。
...
- **クローンの個別メモ管理**:
  - `CloneRecord` およびクローン編集ウィンドウ（`CloneEditWindow`）に専用の `notes`（メモ）フィールドを追加。アカウント名、用途、地域などの補足情報を手軽に記録可能。
  - クローン一覧（`CloneListView`）のテーブルに「メモ」列を追加し、ヘッダークリックによる昇順・降順ソートに対応。
  - ツールバーの検索バーにおいてメモ文字列のあいまい検索に対応し、大量のクローンから目的のインスタンスを即座に絞り込み可能。
  - カード表示（`CloneCard`）において、メモが存在する場合にスニペットをすっきり表示。
  - メモのラベルおよびプレースホルダーに関して全 9 言語のローカライズを完備。

### 📐 メインウィンドウとサイドバーのレイアウト最適化
- **作業領域の拡大と視認性の向上**:
  - メインウィンドウのデフォルトサイズを `1300 x 760`（従来の 1200 x 680 から拡張）に拡大し、テーブルとカードの視認性を改善。
  - 左側サイドバーの幅を 200px から 240px に拡幅し、ナビゲーションラベル、クローン数のバッジ、アップデート状態の表示を快適に。

### 🔬 動的ライブラリの Clang 実コンパイル検証テスト
- **低層フックの信頼性向上**:
  - 動的リンクライブラリに対する、実際の `clang` コンパイラを呼び出す自動統合テストを追加:
    - Cocoa / POSIX interpose フックライブラリ（`libatbclone_hook`）。
    - Lark（Feishu）および ChatGPT 専用の多重起動分離ライブラリ。
    - 環境変数注入ライブラリ（`libatbclone_env`）。
  - マクロやヘッダーファイルの変更によるコンパイル破損やロード時障害を確実に防止。

### 🧪 テストスイート拡充と画面非依存アサーション
- **CI およびテスト実行の安定化**:
  - GUI 統合テストにおける画面解像度に依存するウィンドウサイズのアサーションを整理し、ヘッドレス環境や多様なディスプレイ構成での安定した検証を担保。
  - 自動テストスイートを計 **648 件**（合格率 100%）に拡大。

---

## [v2.1.1] - 2026-09-18

### 🔧 C 言語ランチャーの変数スコープ修正とコンパイル堅牢化
- **Lark / ChatGPT などのクローン起動時のコンパイルエラー解消**:
  - 生成される C ランチャーのソースコードにおいて、動的ライブラリ hook（Lark / Feishu や ChatGPT など）用の `hook_block` 内で宣言された `dir` 変数がランチャー本体のローカル変数と競合し、二重定義エラーが発生していた不具合を修正。
  - `hook_block` を独立したブロック `{ ... }` で囲むようコード生成器を改善し、変数名の衝突を根本的に防止。
  - 単体テストにおいて、実際の `clang` による Lark および ChatGPT ランチャーのコンパイル検証テストを追加。

### 🛡️ プロセス名変更処理の安全性強化と非標準バンドルの耐性向上
- **フォールバックとエラーハンドリングの強化**:
  - 実行バイナリのリネームおよび `Info.plist` 同期処理を強化し、非標準バンドル構造、plist 欠損、破損バイナリなどの異常時にも安全に対処。
  - 例外発生時には警告ログを出力しつつ元の実行パスへ安全にフォールバックし、クローン作成フローの中断を回避。
  - 不正なバンドル構造に対する網羅的なテストケースを追加。

### 🎨 ウィザードアイコンプレビューの Null 安全性向上
- **UI の安定性向上**:
  - クローン作成ウィザード（`WizardWindow`）のアイコン非同期読み込み処理に防御的チェックを追加し、メタデータが不完全なアプリの検出時にも UI エラーが発生しないよう保護。

### 🧪 包括的な品質保証
- **テストスイートの拡張**:
  - 自動テストスイートは計 **645 件**（合格率 100%）に拡大。

---

## [v2.1.0] - 2026-09-17

### 🚀 ハードクローンの個別プロセス名付与とプロキシルールルーティング対応
- **プロセス名ベースのネットワークプロキシ制御**:
  - ハードクローン（Hard Clone）エンジンにおいて、クローンごとに固有のバイナリ実行ファイル名を自動生成し、`CFBundleExecutable` を同期更新（例: `<AppName>_<CloneName>`）。
  - Clash、Surge、Loon、Stash、Quantumult X などのプロキシ管理ツールにおいて、プロセス名に基づくルーティングルール（`PROCESS-NAME,<プロセス名>,<ポリシー>`）を完全サポートし、クローンごとの独立したプロキシ割り当てが可能に。
  - Electron および Chromium アーキテクチャのヘルパープロセス（Helper App）に完全対応し、IPC 通信の整合性を維持しつつ実行ファイル名の競合を防止。
  - クローン編集、アップデート、再生成時にも、個別プロセス名とカスタムアイコンを確実に自動保持。

### 🎨 クローン作成ウィザードでのカスタムアプリアイコン指定
- **柔軟なアプリアイコンのカスタマイズ**:
  - クローン作成ウィザード（`WizardWindow`）にカスタムアイコン選択機能を追加し、`.icns` および `.png` ファイルの適用に対応。
  - ウィザード画面上でのリアルタイムプレビューおよびワンクリックでのデフォルト復元をサポート。
  - リソースディレクトリ（`Contents/Resources/`）内のアイコン置換と設定を自動化し、アップデート後も永続保持。
  - 全 9 言語の UI ラベルおよびボタン文言を完備。

### 🔄 サイドバー最下部へのアップデート機能移設と進捗表示
- **シームレスなアップデート確認体験**:
  - アップデート確認機能を設定画面からナビゲーションサイドバー最下部の常駐エリアへ移設。
  - アニメーション付き更新アイコン、リアルタイムなバージョン情報表示、およびダウンロード時のパーセンテージ進捗バーを統合。
  - 設定画面をシンプルに整理し、一般的な環境設定に集中できる構成へ改善。

### 🧪 包括的な品質保証と自動テスト拡充
- **広範なテストカバレッジ**:
  - プロセス名変更、Helper パス保持、アイコン更新、およびサイドバー更新 UI のテストを追加（`test_process_names.py`、`test_update_icon.py`）。
  - 自動テストスイートは計 **642 件**（合格率 100%）に拡大。

---

## [v2.0.0] - 2026-09-17

### 🏥 環境健全性診断 (Doctor) とディレクトリ権限の検証
- **重要ストレージディレクトリの動的アクセス検証**:
  - 環境診断サブシステム（CLI `atbclone doctor` および GUI `DoctorView`）を大幅に強化し、アプリケーションディレクトリ（`~/ATBClone/Apps`）およびデータ保存ディレクトリ（`~/ATBClone/Data`）のリアルタイム読み書き権限チェックを追加。
  - 安全な一時プローブファイルを用いた事前検証により、クローン作成や起動前に権限不足を検出し、予期せぬ失敗を未然に防止。
  - 詳細なステータス表示、エラーログの明示、および実践的な修復手順（例: `chmod u+rwx <path>`）を提供。
  - `core.config` に `check_directory_access()`、`get_apps_dir()`、`get_data_dir()` ヘルパー関数を追加。
  - 9 言語すべてに対応する診断ラベル、詳細テキスト、修正ガイダンスを完備。

### 📐 公式ドキュメントポータルの KaTeX 数式レンダリングと書式最適化
- **数式タイポグラフィのサポート**:
  - 公式ドキュメントサイト（`clone.aitobox.com`）の全 9 言語版に KaTeX 数式レンダリングエンジンを導入し、技術仕様書の数式表示を最適化。
  - LaTeX ディスプレイ数式のスペーシングとレンダリング品質を向上。

### ⚡ アーキテクチャの成熟と品質保証 (v2.0.0 Milestone)
- **APFS 高性能エンジンと並行性の確立**:
  - v2.0 の基盤を確立：APFS コピーオンライト（`cp -Rc`）による即時クローン、SIMD 加速バイナリ解析、`flock` によるアトミック状態永続化。
  - ディレクトリ診断の単体テストおよび GUI テストを拡充（`test_cmd_doctor.py`、`test_config.py`、`test_probe_and_doctor_ui.py`）。
  - 自動テストスイートは計 **614 件**（合格率 100%）に拡大。

---

## [v1.9.0] - 2026-09-16

### ⚡ APFS コピーオンライト (CoW) による瞬時複製とエンジン性能向上
- **ゼロ追加ストレージと高速クローン**:
  - macOS APFS ファイルシステム上でコピーオンライト（`cp -Rc`）クローンメカニズムを実装し、ギガバイト規模の大規模アプリでもディスク容量を消費せずに瞬時に複製可能に。
  - 非 APFS ボリュームでのフォールバック機能：従来の再帰的コピー（`cp -R`）へ安全に切り替え、フォールバック前に不完全なディレクトリを自動削除して階層構造の破損を防止。
  - ハードウェア SIMD プリフィルタによるバイナリ文字列解析の高速化を実現し、起動引数や環境変数の検出効率を大幅に向上。
  - Mach-O バイナリの適切なスキップによるスクリプト誤判定の防止および安全な Entitlements 整理処理。

### 🔒 プロセス間ファイルロックとアトミックな設定永続化
- **競合状態の排除とデータ整合性の保証**:
  - `flock` ベースのプロセス間ファイルロックを導入し、複数クローン操作や CLI コマンドの同時実行時でも状態ファイルの整合性を完全に保持。
  - 一時ファイル作成、`fsync` によるディスク同期、`os.replace` を組み合わせたアトミック書き込みを採用し、権限を安全な `0o600` に制限。
  - Cocoa interpose 動的ライブラリ hook 内でスレッドローカルな `fake_pw` を使用し、マルチスレッド環境での安全性を確保。

### 🖥️ GUI / HIG の洗練とメモリリークの解消
- **macOS ネイティブ体験と安定性強化**:
  - `LogsView` におけるログバッファ上限設定とメインスレッドへの安全な非同期ディスパッチによりメモリリークを解消。
  - アプリ終了ライフサイクルを強化し、シャットダウン時にリアルタイムログリスナーを自動登録解除。
  - ウィザード、編集、詳細などのサブウィンドウを macOS 標準ウィンドウレベル（`0`）に修正し、他アプリの上に不自然にフロートし続ける問題を解消。
  - ダークモードのコントラスト改善、カスタム `base_dir` の保持、クローン削除時の Keychain プロキシ認証情報の確実な消去。
  - 中国語製品名を **ATB分身助手** に統一。

### 📚 9言語対応の公式ドキュメントサイトと品質保証
- **公式ドキュメントポータル (`clone.aitobox.com`)**:
  - Zensical テーマを採用した静的ドキュメントサイトを開設し、GitHub Pages 連携による自動デプロイを構築。
  - 全 9 言語のドキュメントマトリクスを整備し、統一ナビゲーションおよび UI スクリーンショットを追加。
  - 22 件のリグレッションテストを追加し、テストスイートは計 **610 件**（合格率 100%）に拡張。
