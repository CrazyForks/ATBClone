...
### 🔄 側邊欄更新檢查狀態單行防抖與視覺版面加固
- **杜絕介面垂直跳動與撐高**：
  - 全面重構側邊欄底部版本更新檢查的狀態反饋機制，徹底解決在無網路、DNS 失敗或連線逾時等異常情境下，多行錯誤堆疊撐大底部容器導致的側邊欄高度跳動與視覺錯位。
  - 採用原生 Cocoa `NSLineBreakByTruncatingTail` 單行尾部省略模式配合固定高度標籤，嚴格鎖定側邊欄幾何尺寸。
  - 新增智慧錯誤解析器（`format_update_error`），將複雜的底層 requests 異常提煉為簡短明瞭的本地化單行提示（包括網路逾時、連線異常、HTTP 狀態碼等）。
  - 引入 5 秒自動淡出計時器，錯誤提示展示後平滑復原，保持側邊欄常態清爽整潔。

### 🌐 多語言更新錯誤提示全量適配
- **9 種語言精準覆蓋**：
  - 在全語言字典中補齊 4 項更新錯誤專用翻譯欄位（`update_error_short`、`update_error_timeout`、`update_error_network`、`update_error_http`）。

### 🧪 自動化測試套件擴充與品質保障
- **測試覆蓋更完備**：
  - 新增針對側邊欄更新異常格式化、單行截斷與非同步清理的專項測試套件（`test_sidebar_update.py`、`test_i18n_update_keys.py`）。
  - 全量自動化測試案例擴充至 **659 項**，保持 100% 全部通過。
...
### 📝 分身備忘備註與多維模糊搜尋
- **分身資訊個人化備註**：
  - 在分身資料模型（`CloneRecord`）與分身編輯視窗（`CloneEditWindow`）中新增專屬 `notes` 備註欄位，方便記錄帳號名稱、用途情境、所屬地區或網路環境等資訊。
  - 在分身列表檢視（`CloneListView`）表格中新增獨立的「備註」欄，支援點擊表頭進行正序/倒序排列。
  - 頂部工具列搜尋框全面支援對分身備註進行模糊搜尋匹配，海量分身瞬間定位。
  - 卡片檢視（`CloneCard`）在有備註時自動顯示備註摘要，一目了然。
  - 完整提供 9 種語言的備註標籤與輸入佔位符本地化支援。

### 📐 主視窗與側邊欄版面人體工學最佳化
- **介面空間與呼吸感提升**：
  - 將主視窗預設尺寸擴充至 `1300 x 760`（原 1200 x 680），為卡片網格與多列表格提供更加寬敞舒展的展示空間。
  - 左側導覽側邊欄寬度由 200px 擴展至 240px，使導覽項標籤、分身數量角標以及底部版本更新狀態顯示更從容自然。

### 🔬 動態注入庫 Clang 真實編譯自動化驗證
- **底層 Hook 安全性加固**：
  - 在單元測試套件中新增針對底層動態連結庫真實呼叫 `clang` 編譯器的端對端測試：
    - Cocoa / POSIX interpose 攔截庫（`libatbclone_hook`）。
    - 飛書（Lark）與 ChatGPT 專用多開隔離動態庫。
    - 環境變數注入動態庫（`libatbclone_env`）。
  - 杜絕因語法、編譯巨集或標頭檔變動導致的底層動態庫建置及載入異常。

### 🧪 自動化測試套件擴充與跨顯示器測試解耦
- **測試穩健性進一步加強**：
  - 解耦 GUI 整合測試中依賴具體實體顯示器解析度的視窗尺寸斷言，確保在 CI 無頭執行及不同解析度外接顯示器下測試均穩定可靠。
  - 全量自動化測試案例擴充至 **648 項**，保持 100% 全部通過。

---

## [v2.1.1] - 2026-09-18

### 🔧 C 語言引導啟動器變數作用域修復與編譯加固
- **解決飛書 / ChatGPT 等分身啟動器編譯報錯**：
  - 修復產生 C 語言 Launcher 原始碼時，由於 `hook_block` 插入的程式碼區塊與啟動器自身的 `dir` 區域變數處於同一作用域導致的變數重複定義編譯錯誤（在為飛書、ChatGPT 等注入自訂動態程式庫的應用程式產生分身時觸發）。
  - 在產生器中將 `hook_block` 邏輯包裹在獨立的複合陳述式區塊 `{ ... }` 中，徹底消除變數命名衝突。
  - 在單元測試中新增針對飛書和 ChatGPT 啟動器真實調用 `clang` 編譯的端對端測試，防止編譯迴歸。

### 🛡️ 行程重新命名強健性加固與非標準應用程式套件容錯
- **例外狀況擷取與平滑降級**：
  - 加固獨立二進位可執行檔重新命名與 `Info.plist` 同步邏輯，全面防禦非標準應用程式套件結構、缺少 plist 檔案或損壞二進位檔等異常情境。
  - 遇到異常時平滑降級並輸出明確的警告日誌，絕不中斷分身克隆主流程。
  - 補充針對畸形應用程式套件與降級邏輯的專項測試案例。

### 🎨 分身建立精靈圖示預覽空指標防護
- **介面穩定性提升**：
  - 在建立精靈（`WizardWindow`）圖示預覽非同步載入邏輯中增加防禦性空指標校驗，防止快速掃描應用程式中繼資料不全時觸發 UI 異常。

### 🧪 品質保障與自動化測試套件擴充
- **全方位測試覆蓋**：
  - 自動化測試案例擴充至 **645 項**，保持 100% 全部通過。

---

## [v2.1.0] - 2026-09-17

### 🚀 硬連結分身獨立行程命名與 Clash / 代理規則分流支援
- **基於行程名稱的網路代理精準分流**：
  - 硬連結（Hard Clone）克隆引擎現自動為分身產生唯一的獨立二進位可執行檔名並同步修改 `CFBundleExecutable`（例如 `<應用名稱>_<分身名稱>`）。
  - 完美支援 Clash、Surge、Loon、Stash、Quantumult X 等網路代理工具設定基於行程名稱的策略分流（如 `PROCESS-NAME,<分身行程名稱>,<節點/代理策略>`），實現不同分身獨立分配不同出口 IP 或直連/代理。
  - 深度相容 Electron 與 Chromium 架構輔助程式（Helper App），保留原始輔助套件路徑，杜絕行程名稱衝突並確保 IPC 行程間通訊穩定。
  - 在分身編輯、版本更新或重新產生時，自動完整保留獨立行程名稱與自訂圖示。

### 🎨 分身建立精靈支援自訂應用程式圖示
- **個人化視覺定製體驗**：
  - 在建立精靈（`WizardWindow`）中新增自訂圖示選取功能，支援匯入 `.icns` 與 `.png` 格式圖示。
  - 精靈內提供即時圖示效果預覽，並支援一鍵清除還原預設圖示。
  - 底層引擎自動完成圖示替換與資源目錄（`Contents/Resources/`）配置，並在後續更新中持久化保留。
  - 完整適配全部 9 種語言的圖示選取、清除與提示文案。

### 🔄 側邊欄底部常駐更新入口與進度指示
- **全域可視更新互動**：
  - 將「檢查更新」功能從次要「設定」檢視移轉至側邊欄底部常駐位置，操作更直觀順手。
  - 配備動態重新整理圖示、即時版本回饋以及行內百分比下載進度條，狀態機流轉一目了然。
  - 精簡「設定」頁面排版，使其更聚焦於核心通用設定項目。

### 🧪 品質保障與自動化測試套件擴充
- **全方位測試覆蓋**：
  - 新增獨立行程名稱命名校驗、Helper 路徑相容性、更新圖示持久化與側邊欄更新互動測試（`test_process_names.py`、`test_update_icon.py`）。
  - 自動化測試案例擴充至 **642 項**，保持 100% 通過率。

---

## [v2.0.0] - 2026-09-17

### 🏥 環境健康診斷 (Doctor) 與目錄讀寫權限深度校驗
- **關鍵儲存目錄權限動態探測**：
  - 全面升級環境診斷子系統（命令列 `atbclone doctor` 與 GUI `DoctorView` 視圖），新增針對應用程式目錄（`~/ATBClone/Apps`）與資料儲存目錄（`~/ATBClone/Data`）的即時讀寫權限校驗。
  - 採用安全暫存探針檔案主動探測讀寫權限，在分身生成與實例啟動前提前攔截權限不足風險，杜絕無訊息失敗。
  - 提供細粒度狀態回饋、詳細系統錯誤訊息與直觀的權限修復指引（如 `chmod u+rwx <path>`）。
  - 在 `core.config` 中新增 `check_directory_access()`、`get_apps_dir()` 與 `get_data_dir()` 核心工具函式。
  - 在 i18n 模組中全面補充 9 種語言的診斷提示、狀態標籤與修復建議文案。

### 📐 官方文件入口網 KaTeX 數學公式渲染與排版最佳化
- **數學公式排版支援**：
  - 在官方文件網站（`clone.aitobox.com`）全部 9 個語言版本中深度整合 KaTeX 渲染引擎，完美支援技術原理章節的數學公式排版。
  - 規範並修復 LaTeX 獨立公式間距與渲染樣式，提升技術文件閱讀體驗。

### ⚡ 架構里程碑與品質保證 (v2.0.0 Milestone)
- **APFS 高效能核心與並行安全**：
  - 鞏固 v2.0 架構基石：APFS 寫入時複製（`cp -Rc`）秒級零開銷克隆、硬體 SIMD 字串加速探測、跨行程 `flock` 檔案鎖與不可分割性持久化。
  - 擴充單元測試與 GUI 自動化測試案例（`test_cmd_doctor.py`、`test_config.py`、`test_probe_and_doctor_ui.py`）。
  - 自動化測試套件擴充至 **614 項**，保持 100% 通過率。

---

## [v1.9.0] - 2026-09-16

### ⚡ APFS 寫入時複製 (CoW) 秒級分身與核心效能提升
- **零額外儲存與瞬間複製**：
  - 在 macOS APFS 檔案系統上全面啟用寫入時複製（`cp -Rc`）克隆機制，GB 級大型應用程式分身實現秒級即刻生成，且初期不額外佔用磁碟空間。
  - 具備健全的降級機制：在非 APFS 磁碟區或跨磁碟區操作時平滑降級至傳統遞迴複製（`cp -R`），並在降級前自動清除殘留目錄，徹底杜絕目錄巢狀損毀。
  - 引入硬體 SIMD 預過濾加速二進位字串探測，大幅提升應用程式啟動參數與環境變數分析效能。
  - 智慧跳過 Mach-O 二進位檔案，杜絕指令碼偵測誤報；安全修剪程式碼簽名 entitlements，避免簽名檔案損壞。

### 🔒 跨行程並行鎖與不可分割性狀態持久化
- **徹底杜絕競爭與狀態損毀**：
  - 引入基於 `flock` 的跨行程檔案鎖，確保多開實例並行分身與 CLI 命令並行執行時狀態讀寫嚴格一致。
  - 狀態檔與設定檔全面採用不可分割（原子）寫入機制（暫存檔寫入 + `fsync` 強制同步 + `os.replace` 替換），並嚴格收縮權限至 `0o600`，杜絕寫入中斷導致的資料損毀與未授權讀取。
  - Cocoa interpose 動態程式庫 hook 中採用執行緒局部（thread-local）`fake_pw`，全面保障多執行緒執行安全。

### 🖥️ GUI 規範深度打磨與生命週期記憶體最佳化
- **介面體驗與 macOS HIG 對齊**：
  - 修復 `LogsView` 日誌檢視記憶體佔用無限增長問題，實施環形緩衝限制與主執行緒安全分派。
  - 健全應用程式結束生命週期，在 `_on_app_exit` 中自動取消註冊即時日誌監聽器，釋放背景資源。
  - 規範視窗層級結構：將精靈（Wizard）、分身編輯、規則編輯及詳細資訊視窗還原至 macOS 標準視窗層級（`0`），徹底解決次要對話方塊在切換畫面時常駐浮於其他應用程式之上的違和感。
  - 最佳化深色模式（Dark Mode）對比度，持久化記錄自訂分身根目錄（`base_dir`），並確保 Keychain 代理憑證在分身刪除時自動徹底清除。
  - 統一中文產品官方名稱為 **ATB分身助手**。

### 📚 官方 9 國語言文件站與品質保證
- **全新文件入口網（`clone.aitobox.com`）**：
  - 基於 Zensical 主題建置全新靜態文件網站，並透過 GitHub Pages 自動化流程實現無縫同步發布。
  - 全面擴展涵蓋 9 種語言的完整文件矩陣，統一跨語言導覽並補充介面操作展示截圖。
  - 新增 22 項迴歸與專項測試，自動化測試套件擴充至 **610 項**，保持 100% 通過率。
