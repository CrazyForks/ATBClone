...
### 🔄 Formatage monoligne des erreurs de mise à jour et stabilité de la barre latérale
- **Élimination des décalages verticaux de disposition**:
  - Refonte complète du retour visuel lors de la vérification des mises à jour : évite que de longs messages d'erreur multilignes ne déforment la barre latérale en cas de panne réseau ou de délai dépassé.
  - Utilisation d'étiquettes Cocoa natives monolignes avec troncature finale (`NSLineBreakByTruncatingTail`) et hauteur fixe pour verrouiller les dimensions de la barre latérale.
  - Analyseur d'erreurs intelligent (`format_update_error`) transformant les exceptions réseau et HTTP complexes en statuts concis et localisés sur une seule ligne (délai dépassé, erreur réseau, codes HTTP).
  - Suppression automatique du message d'erreur après 5 secondes pour préserver la sobriété de l'interface.

### 🌐 Traduction intégrale des statuts d'erreur
- **Couverture des 9 langues**:
  - Intégration de 4 clés localisées pour la gestion des erreurs de mise à jour (`update_error_short`, `update_error_timeout`, `update_error_network`, `update_error_http`).

### 🧪 Extension de la suite de tests et fiabilité
- **Tests unitaires dédiés**:
  - Ajout de suites de tests validant le formatage des erreurs, la troncature et la réinitialisation automatique (`test_sidebar_update.py`, `test_i18n_update_keys.py`).
  - Suite de tests portée à **659 tests automatisés** avec un taux de réussite de 100%.
...
- **Contrôle approfondi des accès aux répertoires**:
  - Amélioration du sous-système de diagnostic (CLI `atbclone doctor` et interface `DoctorView`) avec vérification en temps réel des droits de lecture/écriture sur le répertoire des applications (`~/ATBClone/Apps`) et le répertoire des données (`~/ATBClone/Data`).
  - Sondage actif via des jetons temporaires sécurisés pour détecter les refus d'accès avant la duplication ou le lancement d'instances.
  - Rapports détaillés avec indicateurs d'état précis, explications des erreurs système et commandes correctives guidées (`chmod u+rwx <path>`).
  - Nouvelles fonctions utilitaires `check_directory_access()`, `get_apps_dir()` et `get_data_dir()` dans `core.config`.
  - Prise en charge i18n complète des messages de diagnostic et des conseils de remédiation dans les 9 langues.

### 📐 Rendu mathématique KaTeX et mise en page de la documentation
- **Typographie mathématique**:
  - Intégration du moteur KaTeX sur l'ensemble des 9 versions linguistiques du portail officiel (`clone.aitobox.com`).
  - Optimisation des espacements et du rendu des formules LaTeX dans les chapitres d'architecture technique.

### ⚡ Maturité architecturale et assurance qualité (Jalon v2.0.0)
- **Moteur APFS haute performance et synchronisation**:
  - Consolidation des piliers v2.0 : clonage APFS Copy-on-Write (`cp -Rc`) instantané sans duplication de stockage, sondage binaire SIMD et verrous de fichiers `flock`.
  - Élargissement de la couverture de tests unitaires et graphiques pour le diagnostic des répertoires (`test_cmd_doctor.py`, `test_config.py`, `test_probe_and_doctor_ui.py`).
  - Les 614 tests automatisés sont validés à 100%.

---

## [v1.9.0] - 2026-09-16

### ⚡ Clonage APFS Copy-on-Write (CoW) et performances du moteur
- **Clonage instantané sans duplication de stockage**:
  - Activation complète du mécanisme Copy-on-Write (`cp -Rc`) sur les volumes APFS macOS : duplication instantanée d'applications volumineuses sans coût initial d'espace disque.
  - Mécanisme de repli robuste vers la copie récursive standard (`cp -R`) avec nettoyage préalable des dossiers temporaires pour prévenir la corruption par imbrication sur volumes non-APFS.
  - Pré-filtrage accéléré par SIMD matériel pour l'analyse des chaînes binaires, accélérant considérablement la détection des arguments et des variables d'environnement.
  - Ignorance sécurisée des binaires Mach-O lors de l'inspection des scripts wrappers et nettoyage sûr des entitlements de signature.

### 🔒 Verrous de fichiers inter-processus et persistance atomique
- **Élimination complète des conditions de concurrence**:
  - Mise en œuvre de verrous de fichiers basés sur `flock` pour garantir la cohérence des lectures et écritures de statut lors d'opérations simultanées.
  - Écriture atomique des fichiers de configuration et d'état (fichier temporaire + `fsync` + `os.replace`) avec restrictions de permissions à `0o600`.
  - Isolation thread-locale de `fake_pw` dans le hook interpose Cocoa pour une sécurité totale en environnement multithread.

### 🖥️ Polissage de l'interface graphique (HIG) et stabilité mémoire
- **Interface native et cycle de vie renforcé**:
  - Résolution d'une fuite de mémoire dans `LogsView` grâce à un tampon circulaire borné et une transmission sécurisée vers la boucle principale Cocoa.
  - Désenregistrement automatique des écouteurs de logs lors de l'arrêt de l'application (`_on_app_exit`).
  - Rétablissement du niveau de fenêtre standard (`0`) pour les boîtes de dialogue secondaires (Assistant, Édition, Recette, Détails), évitant leur maintien artificiel au premier plan au-dessus des autres applications macOS.
  - Amélioration du contraste en mode sombre, persistance du chemin racine personnalisé (`base_dir`) et suppression automatique des identifiants proxy du trousseau d'accès lors de la suppression d'un clone.
  - Harmonisation du nom de produit en chinois : **ATB分身助手**.

### 📚 Portail de documentation en 9 langues et assurance qualité
- **Nouveau portail de documentation officiel (`clone.aitobox.com`)**:
  - Déploiement d'un site statique moderne avec le thème Zensical et synchronisation automatisée via GitHub Pages.
  - Matrice documentaire complète dans les 9 langues avec navigation multilingue unifiée et captures d'écran guidées.
  - Ajout de 22 tests de non-régression portant la suite à **610 tests** avec un taux de réussite de 100%.
